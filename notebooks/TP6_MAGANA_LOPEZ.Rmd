...
describe_st_point <- function(point, params, ode.func){
  #' Describe a stationnary point
  #' for params, see ?stode
  .x.star <- rootSolve::stode(y=point, parms = params, func = ode.func)
  ...
  .row <- list(
    ...
    `Δ` = .criteria$determinant,
    `δ` = .criteria$discriminant
  )
  as.data.frame(.row)
}
...
* $\frac{\partial f}{\partial x} \rightarrow \partial_x f$
...
dX <- function(X, Y){ kinx * (1 + a*Y^n/(1+Y^n)) - koutx * X }
dY <- function(X, Y){ kiny * (1 + b*X^n/(1+X^n)) - kouty * Y }
...
stability_criteria(x.star1, derivee.solver, params) 
stationnary_type(x.star1, derivee.solver, params) 
stability_criteria(x.star2, derivee.solver, params) 
stationnary_type(x.star2, derivee.solver, params) 
stability_criteria(x.star3, derivee.solver, params) 
stationnary_type(x.star3, derivee.solver, params) 
```
